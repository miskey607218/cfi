...
def parse_df1_layer_chains():
    ...
    instr_csv = os.path.join(build_dir, 'register_dfi_instructions.csv')
    du_csv = os.path.join(build_dir, 'register_dfi_def_use_chains.csv')
...
def main():
    ...
    so_path = os.path.join(build_dir, "test.so")
    ...
    # 解析 CFI 表
    table = parse_cfi_table(os.path.join(build_dir, "test_jump_analysis.csv"))
    for entry in table:
    ...
    # 从 CSV 中获取 test_all 的静态偏移（用于验证模块基址）
    static_offset = None
    for entry in table:
        if entry['src_func'].decode() == "test_all":
            static_offset = entry['src_func_addr']
            break
    if static_offset is None:
        print("错误：无法从 CSV 中找到 test_all 函数起始地址")
        return
    ...
    try:
        while not trigger_done.is_set():
            ...
            if total > 0 and total - last_report >= 10:
                _sys.stdout.write(f"\r  CFI: {event_count} | DFI: {layer_event_count} | 违规: {violation_count}  ")
                _sys.stdout.flush()
                last_report = total
        # 触发完成后等待缓冲写入
