...
def parse_df1_layer_chains(instr_csv_path=None, du_csv_path=None):
    """从 CSV 文件解析间接跳转相关寄存器的三层数据流链"""
    script_dir = os.path.dirname(os.path.abspath(__file__))

    instr_csv = instr_csv_path or os.path.join(script_dir, 'register_dfi_instructions.csv')
    du_csv = du_csv_path or os.path.join(script_dir, 'register_dfi_def_use_chains.csv')
...
OUTPUT_PREFIX = "demo"       # output CSV file prefix
OUTPUT_DIR = "output"        # base output directory
OUTPUT_CFI_FILE = None       # CFI events CSV file path (set in main)
OUTPUT_DFI_FILE = None       # DFI layer events CSV file path (set in main)
OUTPUT_LOG_FILE = None       # events log file path (set in main)
cfi_csv_writer = None
dfi_csv_writer = None
log_file_handle = None

def init_output_files():
    """Initialize CSV output writers for CFI events and DFI layer events."""
    global OUTPUT_CFI_FILE, OUTPUT_DFI_FILE, OUTPUT_LOG_FILE
    global cfi_csv_writer, dfi_csv_writer, log_file_handle

    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_dir = os.path.dirname(script_dir)
    out_base = os.path.join(project_dir, OUTPUT_DIR)
    os.makedirs(out_base, exist_ok=True)

    OUTPUT_CFI_FILE = os.path.join(out_base, f"{OUTPUT_PREFIX}_cfi_events.csv")
    OUTPUT_DFI_FILE = os.path.join(out_base, f"{OUTPUT_PREFIX}_dfi_layers.csv")
    OUTPUT_LOG_FILE = os.path.join(out_base, f"{OUTPUT_PREFIX}_events.log")

    # CFI events CSV
    cfi_f = open(OUTPUT_CFI_FILE, 'w', newline='')
    cfi_csv_writer = csv.DictWriter(cfi_f, fieldnames=[
        'event_id', 'timestamp_ns', 'pid', 'cpu',
        'jump_type', 'jump_type_name', 'src_func', 'dst_func',
    ...
    # DFI layer events CSV
    dfi_f = open(OUTPUT_DFI_FILE, 'w', newline='')
    dfi_csv_writer = csv.DictWriter(dfi_f, fieldnames=[
        'event_id', 'timestamp_ns', 'pid', 'cpu',
        'site_id', 'layer', 'func_name',
        'inst_offset', 'reg_value', 'target_addr',
        'instr_type', 'instr_type_name'
    ])
    dfi_csv_writer.writeheader()

    # Log file
    log_file_handle = open(OUTPUT_LOG_FILE, 'w')

    print(f"📁 输出文件已初始化:")
    print(f"   CFI 事件: {OUTPUT_CFI_FILE}")
    print(f"   DFI 层:   {OUTPUT_DFI_FILE}")
    print(f"   日志:     {OUTPUT_LOG_FILE}")
...
def handle_jump_event(cpu, data, size):
    ...
    if event.is_correct:
        ...
        # ── 基本CFI违规立即终止（enforce模式）──
        if CFI_MODE == 'enforce':
            print(f"\n{'='*60}")
            print(f"  🛑🛑🛑 CFI违规! 立即终止进程 🛑🛑🛑")
            print(f"{'='*60}")
            print(f"  📋 违规详情:")
            print(f"     跳转类型:   {jump_type_name}")
            print(f"     源函数:     {src_func_str}")
            print(f"     目标函数:   {dst_func_str if dst_func_str else '(未知)'}")
            if event.jump_type in (4, 5):
                print(f"     RAX(实际):  0x{event.reg_rax:016x}")
                print(f"     saved_rax:  0x{event.saved_rax_val:016x}")
                if event.saved_rax_val and event.reg_rax != event.saved_rax_val:
                    print(f"     🔴 saved_rax ≠ rax → 函数指针被劫持!")
            elif event.jump_type == 3:
                print(f"     返回地址:   0x{event.ret_addr:016x}")
                print(f"     saved_rsp:  0x{event.saved_rsp_val:016x}")
                if event.saved_rsp_val and event.ret_addr != event.saved_rsp_val:
                    print(f"     🔴 saved_rsp ≠ ret_addr → 返回地址被劫持!")
            print(f"     指令偏移:   0x{event.src_offset:x}")
            print(f"     运行时IP:   0x{event.runtime_ip:016x}")
    ...
    if cfi_csv_writer is not None:
        cfi_csv_writer.writerow({
            ...
            'is_correct': 1 if event.is_correct else 0,
...
def handle_df1_layer_event(cpu, data, size):
    ...
    # ── Write to CSV ──
    global dfi_csv_writer
    if dfi_csv_writer is not None:
        instr_type_names = {0: "DIRECT", 1: "DEREF", 2: "RIP_REL", 3: "RBP_REL"}
        dfi_csv_writer.writerow({
            ...
            'reg_value': f"0x{event.reg_value:x}",
            'target_addr': f"0x{event.target_addr:x}",
            'instr_type': event.instr_type,
            'instr_type_name': instr_type_names.get(event.instr_type, f"TYPE_{event.instr_type}"),
        })
...
def main():
    ...
    global origin_allowed_targets, ptr_origin_map_py
    global CFI_MODE, CFI_METHOD_CONFIG
    global OUTPUT_PREFIX, OUTPUT_DIR, cfi_csv_writer, dfi_csv_writer, log_file_handle
    ...
    parser.add_argument('--so', default=None,
                        help='Path to .so shared library (default: src/test.so)')
    parser.add_argument('--csv', default=None,
                        help='Path to jump analysis CSV (default: src/test_jump_analysis.csv)')
    parser.add_argument('-o', '--output', default='demo',
                        help='Output prefix for CSV files (default: demo)')
    parser.add_argument('-n', '--iterations', type=int, default=0,
                        help='Number of test iterations (0=infinite, default: 0)')
    parser.add_argument('--entry', default='test_all',
                        help='Entry function symbol to call (default: test_all)')
    ...
    # ── Path configuration ──
    so_path = args.so if args.so else os.path.join(script_dir, "test.so")
    csv_path = args.csv if args.csv else os.path.join(script_dir, "test_jump_analysis.csv")
    OUTPUT_PREFIX = args.output
    training_json = os.path.join(project_dir, "cfi_training_data.json")

    so_name = os.path.basename(so_path)

    # Derive DFI CSV paths from the jump analysis CSV directory
    csv_dir = os.path.dirname(os.path.abspath(csv_path))
    csv_base = os.path.splitext(os.path.basename(csv_path))[0]  # e.g. "test_jump_analysis"
    dfi_base = csv_base.replace('_jump_analysis', '_register_dfi')
    instr_csv_derived = os.path.join(csv_dir, f'{dfi_base}_instructions.csv')
    du_csv_derived = os.path.join(csv_dir, f'{dfi_base}_def_use_chains.csv')
    ...
    print(f"SO 文件:  {so_path}")
    print(f"CSV 文件: {csv_path}")
    print(f"入口函数: {args.entry}")
    print(f"迭代次数: {args.iterations if args.iterations > 0 else '无限'}")
    print(f"{'='*60}")

    # 解析三层 DFI 链
    print("\n📊 解析三层 DFI 数据流链...")
    layer_chains, func_bases = parse_df1_layer_chains(
        instr_csv_derived if os.path.exists(instr_csv_derived) else None,
        du_csv_derived if os.path.exists(du_csv_derived) else None
    )
    ...
    # 从 CSV 中获取第一个函数作为入口函数锚点
    static_offset = None
    for entry in table:
        fn_name = entry['src_func'].decode() if isinstance(entry['src_func'], bytes) else entry['src_func']
        if fn_name == args.entry:
            static_offset = entry['src_func_addr']
            break
    if static_offset is None and table:
        # Use first entry's func_addr as fallback
        static_offset = table[0]['src_func_addr']
    if static_offset is None:
        print("错误：无法从 CSV 中找到任何函数起始地址")
        return
    ...
    try:
        entry_func = getattr(lib, args.entry)
    except AttributeError:
        # Try finding any function
        func_names = [e['src_func'].decode() if isinstance(e['src_func'], bytes) else e['src_func']
                      for e in table if e.get('src_func')]
        if func_names:
            try:
                entry_func = getattr(lib, func_names[0])
            except AttributeError:
                print(f"错误：无法找到入口函数 {args.entry}")
                return
        else:
            print(f"错误：无法找到入口函数 {args.entry}")
            return

    func_addr = ctypes.cast(entry_func, ctypes.c_void_p).value
    base = get_module_base_from_maps(so_name)
    if base is None:
        base = func_addr & ~0xFFF  # Fallback: use page-aligned entry address
    print(f"检测到 {so_name} 基址: 0x{base:x}")
    ...
    iteration_count = [0]  # list for mutable closure

    def trigger():
        print(f"🚀 开始执行: {args.entry}() [最多 {max_iterations} 次]" if max_iterations > 0
              else f"🚀 开始执行: {args.entry}() [无限循环]")
        while True:
            entry_func()
            iteration_count[0] += 1
            if max_iterations > 0 and iteration_count[0] >= max_iterations:
    ...
    if max_iterations > 0:
        print(f"将在 {max_iterations} 次迭代后自动停止")
    print("按 Ctrl+C 停止\n")

    # 空闲检测：事件停止到达后立刻退出
    start_time = time.time()
    max_timeout = 30 if max_iterations > 0 else None  # 硬超时保底
    idle_timeout = 2.0                                 # 2 秒无新事件即退出
    last_total = 0
    ...
    try:
        while True:
            ...
            if max_iterations > 0:
                ...
                if total != last_total:
                    last_total = total
                    last_change_time = now   # 有新事件，重置空闲计时
                elif now - last_change_time > idle_timeout:
                    print(f"✅ 监控完成 ({total} 事件, {now - start_time:.1f}s)")
                    break
                if now - start_time > max_timeout:
                    print(f"⏰ 监控超时 ({max_timeout}s)")
                    break
        ...
        if CFI_MODE == 'train':
            try:
                ...
                print(f"✅ 训练数据已保存到 {training_json}")
            except Exception as e:
                print(f"⚠️ 训练数据保存失败: {e}")

        # Close CSV writers and log file
        global log_file_handle
        if log_file_handle:
            try:
                # Write final stats to log
                log_file_handle.write(f"\n=== 最终统计 ===\n")
                log_file_handle.write(f"CFI模式: {CFI_MODE}\n")
                log_file_handle.write(f"处理事件数: {event_count}\n")
                log_file_handle.write(f"CFI违规数: {violation_count}\n")
                log_file_handle.write(f"三层DFI事件数: {layer_event_count}\n")
