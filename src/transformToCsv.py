...
# ===================== 配置项（可通过命令行覆盖） =====================
_BUILD_DIR = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), '..', 'build', 'test')
DEFAULT_DISASM_FILE = _os.path.join(_BUILD_DIR, "test.txt")
DEFAULT_OUTPUT_CSV = _os.path.join(_BUILD_DIR, "test_jump_analysis.csv")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='objdump → CSV 跳转指令分析')
    parser.add_argument('--input', '-i', default=DEFAULT_DISASM_FILE,
                        help=f'输入反汇编文件 (默认: {DEFAULT_DISASM_FILE})')
    parser.add_argument('--output', '-o', default=DEFAULT_OUTPUT_CSV,
                        help=f'输出 CSV 文件 (默认: {DEFAULT_OUTPUT_CSV})')
    args = parser.parse_args()

    DISASM_FILE = args.input
