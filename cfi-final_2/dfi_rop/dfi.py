...
static inline int dfi_do_probe(struct pt_regs *ctx, u32 layer,
    ...
    // 只记录最新一次修改值，key 仅用 site_id，后续更新覆盖之前的值
    u64 key = (u64)meta->site_id;
    dfi_layer_vals.update(&key, &reg_val);

    // 将计算出的正确目标存入 saved_rax，供 trace_all_jumps 校验
...
def handle_df1_layer_event(cpu, data, size):
    ...
    if event.target_addr != 0:
        ...
        if event.layer == 1:
            ...
            print(f"      🔍 **(内存) → 目标地址")
    elif event.reg_value != 0:
        print(f"  🎯 跳转目标:  0x{event.reg_value:016x}")
    else:
        print(f"  🎯 跳转目标:  (计算失败或无效)")
...
def main():
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    log_path = os.path.join(script_dir, "analysis_result.txt")
    ...
    # 解析 CFI 表
    cfi_csv_path = os.path.join(script_dir, "test_jump_analysis.csv")
    table = parse_cfi_table(cfi_csv_path)
    for entry in table:
