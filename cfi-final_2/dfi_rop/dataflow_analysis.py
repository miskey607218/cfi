"""
寄存器数据流完整性（Register DFI）分析器 - v2
=================================================

【程序作用】
  本程序对 x86-64 反汇编代码进行寄存器级别的数据流分析，主要用于检测
  间接跳转/间接调用/ret 指令的目标地址是否存在数据流完整性（DFI）违规。

  核心分析流程：
    1. 解析 objdump -d 输出的反汇编文本，提取每条指令的助记符、操作数、
       源寄存器（USE）、目标寄存器（DEF）。
    2. 构建每个函数的基本块控制流图（CFG），处理条件跳转、直接跳转、
       间接跳转以及 ret 指令的后继/前驱关系。
    3. 对每个函数独立执行迭代式到达定义分析（reaching definitions），
       在控制流汇合点做集合并（union），得到每个指令位置各寄存器的
       定义来源集合。
    4. 对间接调用/间接跳转指令，向后追踪目标寄存器的定义链（最多3层），
       判断目标是本地定义、跨函数定义还是未定义。
    5. 对 ret 指令，追踪 rsp → rbp → 栈帧链，分析返回地址的来源。
    6. 输出两个 CSV 文件：指令级分析表 + def-use 链表，标注 DFI 违规。

  检测的违规类型：
    - 跨函数使用（cross-function use）：寄存器定义来自其他函数
    - 未定义使用（use-before-def）：寄存器没有可达定义
    - 间接跳转目标寄存器来源不明（参数/外部定义）

【运行方法】
  python3 dataflow_analysis.py [输入反汇编文件] [输出前缀]

  参数：
    输入反汇编文件  — objdump -d 的输出文件，默认为 test.txt
    输出前缀        — 生成的 CSV 文件前缀，默认为 register_dfi
  ...
  示例：
    ...
    # 3. 输出文件
    #   register_dfi_instructions.csv    — 每条指令的寄存器使用/定义及违规信息
    #   register_dfi_def_use_chains.csv  — 寄存器 def-use 链详情

【设计决策（v2 相对 v1 的改进）】
  1. 跨函数 def-use 链：移除 global_last_def，每个函数独立分析。
     参数寄存器在函数入口处获得合成定义 <param:REG>。
  2. 函数参数：rdi/rsi/rdx/rcx/r8/r9 在函数入口以合成定义初始化
     （遵循 x86-64 System V ABI 调用约定）。
  3. call 建模：call 指令将调用者保存寄存器标记为 clobbered（kill），
     仅定义 rax（返回值）。rdi/rsi/rdx/rcx/r8/r9 视为 USE（传参）。
  4. 控制流：基于基本块构建 CFG，解析分支目标地址，运行迭代式
     到达定义分析（过程内，汇合点取并集）。
  5. 间接控制流分类：
      - GOT-based（如 *0xXXXX(%rip)）→ 低风险（PLT/GOT 桩）
      - register-based（如 *%rax）      → 高风险
      - memory-based（如 *(reg)）       → 严重风险
      - ret                             → 高风险（返回地址在 [rsp]）
"""
...
def parse_operands(mnemonic: str, operands: str) -> tuple:
    ...
    if len(parts) == 2:
        ...
        if m in ('add','sub','and','or','xor','shl','shr','sar',
                 'ror','rol','adc','sbb','neg','bsf','bsr'):
            if dst_is_mem:
                return list(dict.fromkeys(src_regs + dst_regs)), [], []
            return list(dict.fromkeys(src_regs + dst_regs)), dst_regs[-1:] + ['eflags'], []

        # arithmetic without flags: not, imul, popcnt, tzcnt, lzcnt
        if m in ('not','imul','popcnt','tzcnt','lzcnt'):
            if dst_is_mem:
                return list(dict.fromkeys(src_regs + dst_regs)), [], []
        ...
        if m.startswith('cmov'):
            return list(dict.fromkeys(src_regs + dst_regs + ['eflags'])), dst_regs[-1:] if dst_regs else [], []
        ...
        if re.match(r'j[a-z]+', m):
            return ['eflags'], [], []   # conditional jumps USE eflags
