...
def parse_operands(mnemonic: str, operands: str) -> tuple:
    ...
    if not operands:
        if m in ('ret', 'retq'):
            return ['rax'], [], []
        if m in ('nop', 'nopl', 'nopw', 'endbr64', 'cld', 'std'):
            return [], [], []
        if m == 'leave':
            return ['rbp'], ['rsp', 'rbp'], []
        return [], [], []
...
def parse_asm(text: str) -> list:
    ...
    for line in text.splitlines():
        ...
        if m_lower in KNOWN_PREFIXES and operands:
            ...
            if first_word in ('jmp','jmpq','call','callq','ret','retq') or \
        ...
        is_indirect_jump = m_lower in ('jmp','jmpq') and '*' in operands
        is_direct_call = m_lower in ('call','callq') and not is_indirect_call
        is_direct_jump = m_lower in ('jmp','jmpq') and not is_indirect_jump
        is_conditional_jump = bool(re.match(r'j[a-z]+', m_lower) and
                                   m_lower not in ('jmp','jmpq'))
    ...
    for func, instrs in func_instrs.items():
        for instr in instrs:
            ...
            per_instr.append({
                ...
                'indirect_jump': 'Yes' if instr.is_indirect_jump else 'No',
