#!/usr/bin/env python3
"""
time/dfi.py - BPF/CFI Performance Timing Test
...
# ======================== BPF 程序 (timing版: bpf_send_signal 替换为 return 0) ========================
...
static inline int dfi_do_probe(struct pt_regs *ctx, u32 layer,
    ...
    // timing version: validate but don't kill (avoid ASLR issues with subprocess)
...
def run_forked_test(lib):
    """Fork a child process, run lib.test_all(), return elapsed wall time in seconds."""
    r_fd, w_fd = os.pipe()
    pid = os.fork()
    if pid == 0:
        os.close(r_fd)
...
def setup_bpf(so_path):
    """Parse CSVs, load BPF program, attach uprobes. Returns (bpf_object, lib, base, table)."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    ...
    print(f"[*] 共挂载 {probe_count} 个 uprobe 探针")
    return b, lib, base, table

# ======================== 主程序 ========================

def print_stats(times, label):
    """Print statistics for a single timing run."""
    if not times:
        print(f"\n  [{label}] 无数据")
        return None
...
def main():
    ...
    if no_bpf_only:
        # ---- Run WITHOUT BPF only (clean start) ----
        lib = ctypes.CDLL(so_path)
    ...
    # ---- Normal mode: BPF first, then detach, then no-BPF ----
    b, lib, base, table = setup_bpf(so_path)

    # ---- Step 2: Timing WITH BPF ----
    print(f"\n{'='*60}")
