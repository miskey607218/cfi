#!/usr/bin/env python3
# monitor_insn_plus.py - 监控用户态函数偏移的指令，输出完整 CFI 信息（类似 cfi_final）
from bcc import BPF
import ctypes
import sys
import argparse
...
static int parse_jump_target(struct pt_regs *ctx, u8 *insn_bytes, u64 ip, u64 *target, u8 *len, u8 opcode) {
    ...
    *target = 0;
    ...
    // 间接跳转/调用 (FF /2, /3, /4, /5) - 目标在 RAX 中
    if (first == 0xFF) {
        *len = 2;  // 粗略长度，实际需要解析 modrm，此处简化
        *target = ctx->ax;
        return 1;
    }
...
int trace_insn(struct pt_regs *ctx) {
    ...
    if (!entry) return 0;  // 无 CFI 规则，不处理
    ...
    event.expected_dst = base + entry->dst_addr;   // 预期绝对地址
    ...
    bpf_probe_read(event.src_func, sizeof(event.src_func), entry->src_func);
    bpf_probe_read(event.dst_func, sizeof(event.dst_func), entry->dst_func);
    ...
    // 验证
    if (entry->dst_addr == 0) {
        // 如果预期目标为 0（如返回指令），暂不验证（可扩展）
        event.is_correct = 1;
    } else {
        event.is_correct = (real_target == event.expected_dst);
    }

    // 对于返回指令，尝试从栈顶读取返回地址（作为 dst_offset）
    if (entry->jump_type == 3) {  // RET
        u64 *stack_ptr = (u64 *)PT_REGS_SP(ctx);
        u64 ret_addr;
        bpf_probe_read(&ret_addr, sizeof(ret_addr), stack_ptr);
        event.dst_offset = ret_addr - base;
    } else {
        event.dst_offset = 0;
    }
...
def handle_jump_event(cpu, data, size):
    ...
    if instr_bytes and instr_bytes != '未知' and instr_len > 1:
        ...
        new_insn_bytes = [csv_first_byte] + list(insn_bytes[:instr_len-1])
...
def main():
    ...
    parser = argparse.ArgumentParser(description='监控用户态函数偏移的指令，输出完整CFI信息')
    parser.add_argument('library', help='共享库路径（如 ./test.so）')
    parser.add_argument('symbol', help='符号名（如 test_indirect_call）')
    parser.add_argument('offset', nargs='?', default='0', type=str,
                        help='符号内的偏移（支持 0x21 或 33），默认 0')
    parser.add_argument('--hex', action='store_true', help='偏移以十六进制解析（推荐）')
    parser.add_argument('--csv', default='test_jump_analysis.csv',
                        help='CFI 规则 CSV 文件路径（默认 test_jump_analysis.csv）')
    args = parser.parse_args()

    # 解析偏移
    try:
        if args.hex or (args.offset.lower().startswith('0x')):
            offset = int(args.offset, 16)
        else:
            offset = int(args.offset, 0)
    except ValueError:
        print(f"❌ 偏移格式错误: {args.offset}")
        sys.exit(1)

    so_path = args.library
    ...
    # 从 CSV 中获取第一个条目的静态偏移（用于计算基址）
    if len(table) == 0:
        print("错误：CSV 中没有有效规则")
        return
    # 选择一个已知函数（例如第一个条目的 src_func）来计算基址
    # 这里使用第一个条目的 src_func_addr 作为校准点
    static_offset = table[0]['src_func_addr']
    # 需要知道该函数在运行时的绝对地址。我们通过库和符号获取，但可能用户提供的符号不是该函数。
    # 为了通用，我们可以尝试从 CSV 中找一个在库中确实存在的函数（如 test_returns），
    # 这里简化：要求 CSV 中包含一个名为 "test_returns" 的函数，与 cfi_final 一致。
    # 否则，我们使用用户提供的符号（但该符号可能不在 CSV 中）。
    # 更稳健的做法：从 CSV 中找到一个在库中存在的函数，这里我们假设 CSV 中有 test_returns。
    static_func = None
    for entry in table:
    ...
    if static_func is None:
        print("错误：CSV 中未找到 'test_returns' 函数，无法自动计算基址。")
        print("请确保 CSV 包含该函数，或手动提供基址。")
        return
    static_offset = static_func['src_func_addr']

    # 加载 BPF 程序
    print("\n加载 BPF 程序...")
    b = BPF(text=get_bpf_text())

    # 计算模块基址（使用 test_returns 的运行时地址）
    lib = ctypes.CDLL(so_path)
    try:
        func_addr = ctypes.cast(getattr(lib, "test_returns"), ctypes.c_void_p).value
    except AttributeError:
        print("错误：库中未找到符号 'test_returns'，无法自动计算基址。")
        return
    base = func_addr - static_offset
    print(f"检测到 {so_path} 基址: 0x{base:x}")
    ...
    # 附加 uprobe
    try:
        b.attach_uprobe(name=so_path, sym=args.symbol, sym_off=offset, fn_name="trace_insn")
        print(f"✅ uprobe 附加成功 → {args.symbol}+0x{offset:x}")
    except TypeError:
        # 降级为 addr 方式
        try:
            sym_addr = ctypes.cast(getattr(lib, args.symbol), ctypes.c_void_p).value
            target_addr = sym_addr + offset
            b.attach_uprobe(name=so_path, addr=target_addr, fn_name="trace_insn")
            print(f"⚠️ 使用 addr 模式: 0x{target_addr:x} （可能受 ASLR 影响）")
        except Exception as e:
            print(f"❌ 附加 uprobe 失败: {e}")
            sys.exit(1)

    # 触发函数（可选，如果用户只是监控，不自动调用）
    print("\n=== CFI 监控已启动 ===")
    print(f"监控目标: {args.symbol}+0x{offset:x} @ {so_path}")
    print("按 Ctrl+C 停止\n")
