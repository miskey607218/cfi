...
def get_bpf_text():
    """BPF C program with ROP-aware EC enforcement (func_heads + ret_targets + bpf_send_signal)."""
    return """
...
def handle_jump_event(cpu, data, size):
    ...
    if CFI_MODE in ('enforce', 'hybrid'):
        ...
        if cs_violation or origin_violation:
            ...
            global attack_triggered
            attack_triggered = True
            # Signal main loop to shut down cleanly
...
def main():
    ...
    so_path = os.path.join(default_build_dir, "test.so")
    ...
    print("=" * 60)
    csv_path = os.path.join(default_build_dir, "test_jump_analysis.csv")
    ...
    print(f"[*] {so_basename} 基址: 0x{base:x}")
