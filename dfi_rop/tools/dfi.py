...
def handle_jump_event(cpu, data, size):
    global event_count, violation_count, base, cfi_lookup, last_reg_state
    ...
    if pid_key not in last_reg_state:
        last_reg_state[pid_key] = {}

    changed_count = 0
    for name, val in current_regs.items():
        prev = last_reg_state[pid_key].get(name)
        tag = ""
        if prev is not None and val != prev:
            diff = val - prev if val >= prev else prev - val
            direction = "↗" if val > prev else "↘"
            tag = f"  {direction} 变化: 0x{prev:016x} → 0x{val:016x} (差 0x{diff:x})"
            changed_count += 1
        elif prev is None:
            tag = "  (首次记录)"
        ...
        print(f"  • {name}: 0x{val:016x}{role}{tag}")

        # 更新快照
        last_reg_state[pid_key][name] = val

    if changed_count > 0:
        print(f"  ── {changed_count} 个寄存器值发生变化 ──")
    else:
        print(f"  ── 所有寄存器值未变 ──")

    # 显示保存的 RAX/RSP 校验值
    print(f"  • 保存校验值 (saved_rax): 0x{event.saved_rax_val:016x}")
    print(f"  • 保存校验值 (saved_rsp): 0x{event.saved_rsp_val:016x}")
    # 跳转分析
    print("\n🔍 跳转分析:")
...
REG_NAMES_LIST = ['rax', 'rcx', 'rdx', 'rbx', 'rsp', 'rbp', 'rsi', 'rdi']

def _fmt_val(v):
    """Format a 64-bit value for display."""
    if v is None or v == 0:
        return "0x0000000000000000"
    return f"0x{v:016x}"

def display_register_dataflow_chain(pid, site_id, layers, chain_info):
    """展示一条完整的三层DFI寄存器数据流变换链。

    layers: dict {layer_num: {reg_value, target_addr, inst_offset, timestamp, func_name}}
    chain_info: 静态链配置 (func, reg, layers[...])
    """
    func = chain_info.get('func', '?')
    reg = chain_info.get('reg', '?').upper()
    static_layers = {l['level']: l for l in chain_info.get('layers', [])}

    sorted_layers = sorted(layers.items(), key=lambda x: x[0])  # L1, L2, L3

    # ── 输出变换链头部 ──
    print(f"\n╔{'═'*74}╗")
    print(f"║  寄存器数据流变换链 — site#{site_id}  PID={pid}  func={func[:36]}".ljust(76) + "║")
    print(f"║  追踪寄存器: {reg}".ljust(76) + "║")
    print(f"╠{'═'*74}╣")

    # ── 表头 ──
    print(f"║ {'层':>2} │ {'指令偏移':>8} │ {'指令 (导致寄存器变换)':<33} │ {'寄存器值变换':<24} ║")
    print(f"╠{'═'*74}╣")
    ...
    for layer_num, evt in sorted_layers:
        linfo = static_layers.get(layer_num, {})
        instr_str = linfo.get('instr', '?')
        itype = linfo.get('instr_type', 0)
        itype_name = INSTR_TYPE_NAMES.get(itype, f"TYPE_{itype}")

        rv = evt['reg_value']
        ta = evt['target_addr']
        offset = evt['inst_offset']

        if layer_num == 3 and rv:
            initial_val = rv
        if layer_num == 1:
            final_target = ta or rv

        # ── 构建寄存器变换描述 ──
        if layer_num == 3:
            if itype == 2:       # RIP_REL
                action = f"{reg} ← mem[rip+disp] (读数据段)"
            elif itype == 3:     # RBP_REL
                action = f"{reg} ← mem[rbp+disp] (读栈)"
            elif itype == 1:     # DEREF
                action = f"{reg} ← *(mem) (首次解引用)"
            else:
                action = f"{reg} ← 初始加载"
        elif layer_num == 2:
            if itype == 1:
                action = f"{reg} ← *({reg}) (解引用指针)"
            elif itype == 0:
                action = f"{reg} ← 寄存器传递 (mov/lea)"
            elif itype == 2:
                action = f"{reg} ← mem[rip+disp]"
            elif itype == 3:
                action = f"{reg} ← mem[rbp+disp]"
            else:
                action = f"{reg} ← 中间传递"
        elif layer_num == 1:
            if itype == 0:
                action = f"JUMP → {reg} (寄存器值=目标)"
            elif itype == 4:
                action = f"RET  → *(rsp) (弹出返回地址)"
            else:
                action = f"JUMP → 目标地址"

        # ── 寄存器值变换 ──
        if prev_val is not None and prev_val != 0 and rv != 0:
            if rv == prev_val:
                delta_str = f"{reg} = {_fmt_val(rv)} (未变)"
            else:
                delta_str = f"{reg}: {_fmt_val(prev_val)} → {_fmt_val(rv)}"
        else:
            delta_str = f"{reg} = {_fmt_val(rv)}"

        # ── 打印层信息 ──
        # 第一行：层号 + 偏移 + 指令
        print(f"║ {layer_num:>2} │ 0x{offset:<6x} │ {instr_str:<33} │ {action:<24} ║")

        # 第二行：类型 + 寄存器值
        print(f"║    │ {'类型:':>8} │ {itype_name:<33} │ {delta_str:<24} ║")

        if ta != 0 and ta != rv:
            print(f"║    │          │ 目标地址: 0x{ta:016x}              │                        ║")
    ...
    print(f"╠{'═'*74}╣")
    print(f"║  📊 数据流总结:                                               ║")
    print(f"║     初始加载值 (L3) : {_fmt_val(initial_val):<41} ║")
    print(f"║     最终跳转目标(L1): {_fmt_val(final_target):<41} ║")
    if initial_val != 0 and final_target != 0:
        if initial_val == final_target:
            print(f"║     {reg} 值未变 (直接传递)                                     ║")
        else:
            print(f"║     {reg} 值已变换 (经解引用/内存读取)                            ║")
    print(f"╚{'═'*74}╝")


def handle_df1_layer_event(cpu, data, size):
    global layer_event_count, dfi_chain_buffer, dfi_chain_info, last_reg_state
    ...
    # ── 缓存事件 ──
    if key not in dfi_chain_buffer:
        dfi_chain_buffer[key] = {}
    dfi_chain_buffer[key][event.layer] = {
        'reg_value': event.reg_value,
        'target_addr': event.target_addr,
        'inst_offset': event.inst_offset,
    ...
    # ── 简短实时提示 ──
    layer_icons = {1: "🔵L1", 2: "🟡L2", 3: "🟢L3"}
    icon = layer_icons.get(event.layer, f"L{event.layer}")
    reg_val_str = _fmt_val(event.reg_value) if event.reg_value else "(空)"
    target_str = _fmt_val(event.target_addr) if event.target_addr else ""
    print(f"  [{icon}] site#{site_id} {func_name}: reg={reg_val_str}"
          + (f"  target={target_str}" if target_str else ""))

    # ── 三层到齐则展示完整变换链 ──
    if len(dfi_chain_buffer[key]) >= 3:
        cinfo = dfi_chain_info.get(site_id, {})
        display_register_dataflow_chain(pid, site_id, dfi_chain_buffer[key], cinfo)
        del dfi_chain_buffer[key]
...
def main():
    ...
    global dfi_chain_buffer, dfi_chain_info, last_reg_state
    event_count = 0
    layer_event_count = 0
    violation_count = 0
    cfi_lookup = {}
    dfi_chain_buffer = {}   # (pid, site_id) -> {layer_num: event_data}
    dfi_chain_info = {}     # site_id -> chain static config
    last_reg_state = {}     # pid -> {reg_name: last_value}
    ...
    so_path = os.path.join(src_dir, "test.so")
    ...
    # 构建 site_id → chain_info 索引，供运行时 DFI 链显示使用
    for idx, chain in enumerate(layer_chains):
        dfi_chain_info[idx] = chain
    print(f"✅ 已建立 {len(dfi_chain_info)} 个站点的静态数据流链索引")
